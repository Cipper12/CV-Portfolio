...
module tmds_encoder (
    ...
    // Clear control data if in reset state
    wire [1:0] ctrl = {2{~i_reset}} & i_ctrl;
    
    // If high, send blank data (in reset or in image blank)
    wire blank = i_reset | ~i_display_enable;

    // Calculate a xor value based on if ones dominate the input, break ties on lowest bit
    wire parity = {$countones(i_data), !i_data[0]} > 8;
    
    // Intermediate encode step
    wire [7:0] enc = {{7{parity}} ^ enc[6:0] ^ i_data[7:1], i_data[0]};

    // Calculate # of ones vs # of zeros bit balance
    wire signed [4:0] balance = {4'($countones(enc)), 1'b0} - 5'b01000;
    
    // Keep a record of bit bias of previously sent data
    reg signed [4:0] bias;
    
    // Track from sign bits if balance is going away or towards bias
    wire bias_vs_balance = (bias[4] == balance[4]);

    // Encode pixel color data with at most 5 bit 0<->1 transitions, and update bias count
    always @(posedge i_hdmi_clk) begin
        o_tmds <= blank ? {~ctrl[1], 9'b101010100} ^ {10{ctrl[0]}} :
                         {bias_vs_balance, ~parity, {8{bias_vs_balance}} ^ enc};
        
        bias <= blank ? 0 : 
                5'(bias + ({5{bias_vs_balance}} ^ balance) + 
                {3'b0, bias_vs_balance^parity, bias_vs_balance});
    end
endmodule
...
module hdmi (
    ...
    // Serialize the three 10-bit TMDS channels to three serial 1-bit TMDS streams.
    // (Gowin FPGA Designer/Sipeed Tang Nano 4K specific module)
    wire serial_tmds[3];
    ...
    // Encode the 1-bit serial TMDS streams to Low-voltage differential signaling (LVDS) HDMI output pins.
    // (Gowin FPGA Designer/Sipeed Tang Nano 4K specific module)
